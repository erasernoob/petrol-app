function [P,Plg,Pdm,Pgn,Phk,ECD,Pgnyx,Phkyx,ECDyx,Sk]=Hydro(guiji,lbmx,pailiang,fluidden,n,K,miu,taof,Dw,A1,C1,A2,C2,A3,C3,Rzz,rzz,Lzz,Rzt,rzt,Lzt,L1,d1,L2,d2,L3,d3,L4,d4,Lp,Li,rzzjt,yxmd,H,yx)
data=guiji;
wc=lbmx;
...
nn=ntrans;
Pgn=zeros(nn,1);
Pgn(end)=Phk(end)+dertaPzt;
for i=1:nn-1
    PI2yx=PI2(end+1-i)-PI2(end-i);
    Ppyx=Pp(end+1-i)-Pp(end-i);
    Pgn(end-i)=Pgn(end+1-i)-PI2yx+Ppyx;
end
%ECD
